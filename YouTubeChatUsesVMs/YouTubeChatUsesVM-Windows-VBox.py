...
def vmware_start(vmx_path, gui=True, vmrun_path=None):
    vmrun_path = vmrun_path or VMRUN_PATH
    return subprocess.run([vmrun_path, "-T", "ws", "start", vmx_path, "gui" if gui else "nogui"], capture_output=True, text=True)

def vmware_stop(vmx_path, hard=True, vmrun_path=None):
    vmrun_path = vmrun_path or VMRUN_PATH
    return subprocess.run([vmrun_path, "-T", "ws", "stop", vmx_path, "hard" if hard else "soft"], capture_output=True, text=True)

def vmware_reset(vmx_path, vmrun_path=None):
    vmrun_path = vmrun_path or VMRUN_PATH
    return subprocess.run([vmrun_path, "-T", "ws", "reset", vmx_path, "hard"], capture_output=True, text=True)

def vmware_revert_to_snapshot(vmx_path, snapshot_name, vmrun_path=None):
    vmrun_path = vmrun_path or VMRUN_PATH
    return subprocess.run([vmrun_path, "-T", "ws", "revertToSnapshot", vmx_path, snapshot_name], capture_output=True, text=True)

def vmware_take_snapshot(vmx_path, snapshot_name, vmrun_path=None):
    vmrun_path = vmrun_path or VMRUN_PATH
    return subprocess.run([vmrun_path, "-T", "ws", "snapshot", vmx_path, snapshot_name], capture_output=True, text=True)
...
def vm_pause():
    if current_vm_backend == "vmware":
        return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'pause', VM_NAME], check=True),
                           source="Cmd/pausevm")
...
def vm_unpause():
    if current_vm_backend == "vmware":
        return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'unpause', VM_NAME], check=True),
                           source="Cmd/resumevm")
...
def vm_save_state():
    if current_vm_backend == "vmware":
        return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'suspend', VM_NAME], check=True),
