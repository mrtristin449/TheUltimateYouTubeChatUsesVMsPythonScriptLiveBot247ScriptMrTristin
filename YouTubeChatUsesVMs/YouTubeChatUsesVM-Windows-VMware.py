...
logger.info("="*80)
logger.info("Bot startup - logging initialized")
logger.info("="*80)
...
def _native_dialog_yesno(msg, title):
    ...
    logger.debug(f"_native_dialog_yesno called: title='{title}'")
    if platform.system() == "Windows":
        import ctypes
        MB_YESNO, MB_ICONQUESTION, IDYES = 0x04, 0x20, 6
        try:
            logger.debug("Showing Windows MessageBox dialog")
            result = ctypes.windll.user32.MessageBoxW(0, msg, title, MB_YESNO | MB_ICONQUESTION) == IDYES
            logger.debug(f"Windows dialog result: {result}")
            return result
        except Exception as e:
            logger.error(f"Windows dialog error: {e}", exc_info=True)
            return True
    elif platform.system() == "Darwin":
        try:
            logger.debug("Showing macOS AppleScript dialog")
            ...
            dialog_result = "button returned:Yes" in (result.stdout or "")
            logger.debug(f"macOS dialog result: {dialog_result}")
            return dialog_result
        except Exception as e:
            logger.error(f"macOS dialog error: {e}", exc_info=True)
            return True
    elif platform.system() == "Linux":
        import shutil as _shutil
        try:
            logger.debug("Checking for Linux dialog tools")
            if _shutil.which("zenity"):
                logger.debug("Using zenity for dialog")
                result = subprocess.run(["zenity", "--question", "--title", title, "--text", msg],
                                         capture_output=True, timeout=120)
                dialog_result = result.returncode == 0
                logger.debug(f"zenity dialog result: {dialog_result}")
                return dialog_result
            elif _shutil.which("kdialog"):
                logger.debug("Using kdialog for dialog")
                result = subprocess.run(["kdialog", "--title", title, "--yesno", msg],
                                         capture_output=True, timeout=120)
                dialog_result = result.returncode == 0
                logger.debug(f"kdialog dialog result: {dialog_result}")
                return dialog_result
        except Exception as e:
            logger.error(f"Linux dialog tool error: {e}", exc_info=True)
            return True
        try:
            logger.debug("Falling back to CLI input")
            answer = input(f"[{title}] {msg}\n(y/n): ").strip().lower()
            dialog_result = answer.startswith("y")
            logger.debug(f"CLI input result: {dialog_result}")
            return dialog_result
        except Exception as e:
            logger.error(f"CLI input error: {e}", exc_info=True)
            return True
    else:
...
def _native_dialog_error(msg, title):
    ...
    logger.debug(f"_native_dialog_error called: title='{title}'")
    if platform.system() == "Windows":
        import ctypes
        MB_ICONERROR = 0x10
        try:
            logger.debug("Showing Windows error dialog")
            ctypes.windll.user32.MessageBoxW(0, msg, title, MB_ICONERROR)
        except Exception as e:
            logger.error(f"Windows error dialog failed: {e}", exc_info=True)
    elif platform.system() == "Darwin":
        try:
            logger.debug("Showing macOS error dialog")
            esc_msg = msg.replace('\\', '\\\\').replace('"', '\\"')
            esc_title = title.replace('\\', '\\\\').replace('"', '\\"')
            ...
            logger.error(f"macOS error dialog failed: {e}", exc_info=True)
    elif platform.system() == "Linux":
        import shutil as _shutil
        try:
            logger.debug("Checking for Linux error dialog tools")
            if _shutil.which("zenity"):
                logger.debug("Using zenity for error dialog")
                subprocess.run(["zenity", "--error", "--title", title, "--text", msg],
                                capture_output=True, timeout=60)
                return
            elif _shutil.which("kdialog"):
                logger.debug("Using kdialog for error dialog")
                subprocess.run(["kdialog", "--title", title, "--error", msg],
                                capture_output=True, timeout=60)
                return
        except Exception as e:
            logger.error(f"Linux error dialog tool failed: {e}", exc_info=True)
        logger.info(f"Error: [{title}] {msg}")
        print(f"[{title}] {msg}")
    else:
        logger.warning(f"Unknown platform for error dialog: {platform.system()}")
        print(f"[{title}] {msg}")
...
def _is_admin():
    if platform.system() != "Windows":
        logger.debug("Non-Windows platform, skipping admin check")
...
if platform.system() == "Windows" and not _is_admin():
    logger.info("Not running as admin on Windows, attempting UAC elevation...")
    import ctypes
...
def _generate_relaunch_batch(folder, base_name, flags):
    ...
    logger.info(f"Generating relaunch batch script for {base_name}")
...
"""
    try:
        with open(batch_path, "w", encoding="utf-8") as f:
            f.write(bat)
        logger.info(f"Successfully generated relaunch batch at: {batch_path}")
        return batch_path
    except Exception as e:
        logger.error(f'Could not write batch file: {traceback.format_exc()}')
...
def _generate_relaunch_script_macos(folder, base_name, flags):
    ...
    instead of taskkill."""
    logger.info(f"Generating macOS relaunch script for {base_name}")
    autostart_script = flags.get("autostart_filename", f"{base_name}_autostarteverything.py")
    autostart_path   = os.path.join(folder, autostart_script)
...
"""
    try:
        ...
        logger.info(f"Successfully generated macOS relaunch script at: {script_path}")
        return script_path
    except Exception as e:
        logger.error(f'Could not write macOS relaunch script: {traceback.format_exc()}')
...
def _generate_relaunch_script_linux(folder, base_name, flags):
    ...
    logger.info(f"Generating Linux relaunch script for {base_name}")
    autostart_script = flags.get("autostart_filename", f"{base_name}_autostarteverything.py")
    autostart_path   = os.path.join(folder, autostart_script)
    script_path      = os.path.join(folder, "run_update.sh")
    python_exe       = sys.executable
...
"""
    try:
        with open(script_path, "w", encoding="utf-8") as f:
            f.write(sh)
        os.chmod(script_path, 0o755)
        logger.info(f"Successfully generated Linux relaunch script at: {script_path}")
        return script_path
    except Exception as e:
        logger.error(f'Could not write Linux relaunch script: {traceback.format_exc()}')
        print(f'[AutoUpdate] Could not write relaunch script: Python exception: "{traceback.format_exc()}"')
        return None
...
def trigger_relaunch_pipeline(reason, version_data=None):
    ...
    if version_data is not None:
        ...
        # Local file-edit trigger -- copy the ORIGINAL base file (e.g. YouTubeChatUsesVM-Windows-VMware.py),
        # NOT script_path. On a 2nd+ relaunch, script_path is already the _autostarteverything copy,
        # so copying it would perpetuate that suffixed variant instead of the user's real edited file.
        # base_name already has the suffix stripped by _script_paths().
        base_source = os.path.join(folder, f"{base_name}.py")
        copy_source = base_source if os.path.exists(base_source) else script_path
        try:
            shutil.copyfile(copy_source, os.path.join(folder, autostart_filename))
        except Exception as e:
...
def update_overlay(author=None, message=None, running=None, msg_id=None, platform="", is_mod=False, is_owner=False):
    ...
    if author and message and msg_id and msg_id not in seen_message_ids:
        ...
        overlay_data["chat"].append({"author": str(author), "message": str(message),
                                      ...
                                      "platform": str(platform or ""), "is_mod": bool(is_mod),
...
def _flask_history_payload():
    ...
    for item in overlay_data.get("chat", []):
        ...
        out.append({
            ...
            "is_owner": bool(item.get("is_owner", False)) or (author.lower() == ADMIN_USERNAME.lower() if ADMIN_USERNAME else False),
            "is_mod": bool(item.get("is_mod", False)),
            "is_admin": bool(item.get("is_owner", False)) or (author.lower() == ADMIN_USERNAME.lower() if ADMIN_USERNAME else False),
        })
    return out
...
# Rate-limit identical send errors: (status_code -> last_log_time).
# Prevents the console from flooding with the same 403 quota error every 1.2s.
_yt_relay_last_err_log = {}   # {status_code: timestamp}
_YT_RELAY_ERR_LOG_COOLDOWN = 60   # seconds between repeat logs of the same error code
        ...
        if status == 401:
            ...
            if new_token:
                ...
                if rc2 == 0 and 200 <= status2 < 300:
                    return True
                _now = time.time()
                if _now - _yt_relay_last_err_log.get(status2, 0) >= _YT_RELAY_ERR_LOG_COOLDOWN:
                    _yt_relay_last_err_log[status2] = _now
                    print(f"[YTLogRelay] curl send failed after refresh (HTTP {status2}): {resp_body2[:200]}")
                return False
        _now = time.time()
        if _now - _yt_relay_last_err_log.get(status, 0) >= _YT_RELAY_ERR_LOG_COOLDOWN:
            _yt_relay_last_err_log[status] = _now
            print(f"[YTLogRelay] curl send failed (HTTP {status}): {resp_body[:200]}")
        return False
    except subprocess.TimeoutExpired:
        _now = time.time()
        if _now - _yt_relay_last_err_log.get("timeout", 0) >= _YT_RELAY_ERR_LOG_COOLDOWN:
            _yt_relay_last_err_log["timeout"] = _now
            print("[YTLogRelay] curl send timed out.")
        return False
    except Exception as e:
        _now = time.time()
        if _now - _yt_relay_last_err_log.get("exception", 0) >= _YT_RELAY_ERR_LOG_COOLDOWN:
            _yt_relay_last_err_log["exception"] = _now
...
# Lines generated by Streamer.bot chat must never be sent back to YouTube.
# Otherwise the sequence becomes: Streamer.bot chat -> console -> YT relay ->
# YouTube chat -> Streamer.bot chat, which creates the repeated nested prefixes
# visible as "[Streamer.bot Chat] MrTristin-Two: [Streamer.bot Chat] ...".
_YT_RELAY_BLOCKED_PREFIXES = (
    "[Streamer.bot Chat]",
    "[Streamer.bot Raw]",
    "[Streamer.bot Command]",
)
_yt_relay_recent_lines = {}
_YT_RELAY_DUPLICATE_WINDOW = 15.0


def enqueue_log_line_for_yt_relay(text):
    """Queue console lines for YouTube, excluding chat feedback and duplicates."""
    if not (YT_LOG_RELAY_CONFIG.get("enabled") and text and text.strip()):
        return
    line = text.strip()
    if any(line.startswith(prefix) for prefix in _YT_RELAY_BLOCKED_PREFIXES):
        return

    # ConsoleRedirect.write() can receive the same print fragment more than once.
    # Suppress an identical line briefly so one log entry produces one chat post.
    now = time.time()
    previous = _yt_relay_recent_lines.get(line, 0)
    if now - previous < _YT_RELAY_DUPLICATE_WINDOW:
        return
    _yt_relay_recent_lines[line] = now
    if len(_yt_relay_recent_lines) > 1000:
        cutoff = now - _YT_RELAY_DUPLICATE_WINDOW
        _yt_relay_recent_lines.update({k: v for k, v in _yt_relay_recent_lines.items() if v >= cutoff})
    try:
        _yt_relay_queue.put_nowait(line)
    except Exception:
        pass
...
def _handle_streamerbot_chat(chat_item):
    ...
    update_overlay(author=display, message=msg, msg_id=msg_id,
                   platform=chat_item.get("platform", ""), is_mod=is_mod, is_owner=is_owner)
...
def _video_panel_window_title():
    """Returns the current title of the Video Panel Toplevel window, or None if not open.
    Called from background threads; hops to the Tk thread safely."""
    if _gui_app is None:
        return None
    ...
    def _read():
        try:
            ...
            if win is not None and win.winfo_exists():
                result["title"] = win.title() or "Video Panel"
        except Exception:
            pass
        ready.set()
    try:
...
def obs_update_video_window_capture(source_name: str = "video") -> bool:
    ...
    On Windows, OBS Window Capture identifies windows by title string. This reads
    the current Video Panel window title and pushes it to the named OBS source.
    Returns True on success, False otherwise."""
    if not _obs_connected or not _obs_client or not source_name:
        return False
    title = _video_panel_window_title()
    if not title:
        print("[OBS] Video Panel window not open -- cannot update window capture source.")
        return False
    try:
        try:
            _obs_client.set_input_settings(source_name, {"window": title}, True)
        except TypeError:
            _obs_client.set_input_settings(source_name, {"window": title})
        print(f"[OBS] Window Capture source '{source_name}' → '{title}'")
        return True
    except Exception:
        print(f'[OBS] Could not update source "{source_name}": Python exception: "{traceback.format_exc()}"')
        return False
...
def set_media_request_restriction(enabled: bool, *, note: str = "") -> bool:
    """Enable/disable moderator-only mode for !sr and !vr requests.
    When enabled, only chat moderators (isChatModerator) can queue song/video
    requests. Mod admin commands like !skipsr / !skipvr / !clearsr / !clearvr
    always remain available to mods regardless of this setting."""
    global MEDIA_REQUEST_RESTRICTED
    MEDIA_REQUEST_RESTRICTED = bool(enabled)
    if note:
        console_log("INFO", note)
    return MEDIA_REQUEST_RESTRICTED

def media_request_allowed(username: str = "", *, kind: str = "song", is_mod: bool = False,
                           fallback_restrict: bool | None = None) -> tuple:
    """Gate !sr / !vr requests behind the moderator-only restriction flag.
...
def queue_song_request(raw, user="", is_mod=False, resume_seconds=0.0):
    ...
    if not music_config.get("enabled", False):
        console_log("WARN", f"[Music] Song request from {user} ignored -- music playback is disabled.")
        return None
    allowed, reason = media_request_allowed(user, kind="song", is_mod=is_mod)
    if not allowed:
        console_log("WARN", f"[Restricted] Blocked song request from {user}: {reason}")
...
def queue_video_request(raw, user="", is_mod=False, resume_seconds=0.0):
    ...
    if not video_config.get("enabled", False):
        console_log("WARN", f"[Video] Video request from {user} ignored -- video playback is disabled.")
        return None
    allowed, reason = media_request_allowed(user, kind="video", is_mod=is_mod)
    if not allowed:
        console_log("WARN", f"[Restricted] Blocked video request from {user}: {reason}")
        ...
        while not bot_stop_event.is_set():
            ...
            try:
                for c in self.chat.get().sync_items():
                    ...
                    update_overlay(author=user, message=msg, msg_id=c.id, platform="youtube",
                                   is_mod=is_mod, is_owner=is_owner)
                            ...
                            # ── Music / Video / Soundboard commands (transferred from chatuses.py) ──
                                ...
                                if args.strip():
                                    def _findsr(q=args, u=user):
                                        vid = find_youtube_video_id(q)
                                        if vid:
                                            queue_song_request(vid, u, is_mod=is_mod)
                                        else:
                                            console_log("INFO", f"[findsr] no result for '{q}'")
                                    threading.Thread(target=_findsr, daemon=True).start()
                            elif cmd == 'skipsr':
                                ...
                                if args.strip():
                                    def _findvr(q=args, u=user):
                                        vid = find_youtube_video_id(q)
                                        if vid:
                                            queue_video_request(vid, u, is_mod=is_mod)
                                        else:
                                            console_log("INFO", f"[findvr] no result for '{q}'")
                                    threading.Thread(target=_findvr, daemon=True).start()
                            elif cmd == 'skipvr':
                                ...
                                if is_mod:
                                    if args.strip().lower() in ("on", "1", "true", "enable"):
                                        ...
                                        set_media_request_restriction(not MEDIA_REQUEST_RESTRICTED,
                                                                     note=f"[restrictmedia] restricted mode {'enabled -- only moderators can queue !sr and !vr.' if not MEDIA_REQUEST_RESTRICTED else 'disabled -- !sr and !vr are open again.'}")
                                    post_system_message(f"[restrictmedia] {'enabled' if MEDIA_REQUEST_RESTRICTED else 'disabled'} by {user}.")
                                else:
...
class UltraBotGUI:
    ...
    TEXT     = "#e8e8f0"
    TEXTDIM  = "#8888aa"
    SHIELD_BLUE = "#2aa6ff"   # mod badge colour
    CONSOLE  = "#0a0a14"
    CONTEXT  = "#00e676"
    BORDER   = "#2d2d50"
    _FONT_SIZE = 10
    ...
    def __init__(self, root):
        ...
        self._config_dirty     = False   # set True whenever any config changes
        self._init_in_progress = False   # suppresses saves during __init__ restore

        # Wrap every global save_* function so calling it also marks config dirty.
        # The 0.1s loop then flushes everything within one tick.
        _gui_self = self
        _save_fn_names = [
            "save_custom_commands", "save_flask_config", "save_realpc_config",
            "save_permissions_config", "save_yt_log_relay_config", "save_sound_config",
            "save_mrtristinai_config", "save_multi_stream_config", "save_scheduler_config",
            "save_vnc_config", "save_user_mgmt", "save_obs_config", "save_host_switch_config",
            "save_auto_start_config", "save_gemini_config", "save_reconnect_config",
            "save_os_voting_config", "save_music_config", "save_video_config",
            "save_soundboard_config", "save_internet_config", "save_autostart_everything_config",
        ]
        import sys as _sys
        for _fn_name in _save_fn_names:
            _orig = globals().get(_fn_name)
            if _orig is None:
                continue
            def _make_wrapper(_f, _gui=_gui_self):
                def _wrapper(*args, **kwargs):
                    try:
                        return _f(*args, **kwargs)
                    finally:
                        try:
                            _gui._config_dirty = True
                        except Exception:
                            pass
                return _wrapper
            try:
                globals()[_fn_name] = _make_wrapper(_orig)
            except Exception:
                pass
        ...
        if saved_backend in ("vmware", "vbox"):
            ...
            self._init_in_progress = True   # suppress save inside _on_vm_backend_changed during init
        ...
        # ── Comprehensive 0.1-second autosave loop ──
        # Only writes when _config_dirty is True (set by the monkey-patched save_*
        # wrappers installed above), so idle ticks never touch disk.
        try:
            self.root.after(100, self._periodic_all_config_autosave)
        except Exception:
            pass
    ...
    def _build_main_tab(self, parent):
        ...
        try:
            ...
            for y in range(h):
                ...
                for x in range(w):
                    dx = x - cx; dy = y - cy
                    row.append(blue if dx*dx + dy*dy <= r*r else bg)
    ...
    def _build_obs_tab(self, parent):
        ...
        ttk.Button(btn_row, text="🎬 Update Video Window Capture",
                   style="Dim.TButton",
                   command=lambda: threading.Thread(
                       target=lambda: (
                           obs_update_video_window_capture("video") or
                           self._log("[OBS] Video Panel not open or OBS not connected.")
                       ), daemon=True).start()
                   ).pack(side="left", padx=(10, 0))
        tk.Label(btn_row,
                 text="Points OBS's 'video' Window Capture source at the Video Panel window.",
                 bg=self.BG, fg=self.TEXTDIM, font=("Segoe UI", 8)).pack(side="left", padx=(8, 0))
    ...
    def _on_vm_backend_changed(self):
        ...
        # Skip saving during __init__ -- the full restore hasn't finished yet and
        # vm=self._vm_var.get() would still be "" or a dropdown default, clobbering
        # the saved VM on every relaunch.
    ...
    def _mark_config_dirty(self):
        """Call to signal that a config has changed and needs flushing."""
        self._config_dirty = True

    def _periodic_all_config_autosave(self):
        """Saves ALL JSON config files ~every 0.1 seconds, but only when
        _config_dirty is True (set by monkey-patched save_* wrappers in __init__).
        This means any code path that changes a setting -- Save buttons, OS Voting,
        Admin CMDs, etc. -- automatically flushes all configs to disk within 100ms.
        On relaunch, every JSON is already up-to-date and nothing is forgotten."""
        try:
            if getattr(self, "_config_dirty", False):
                self._config_dirty = False
                # autostarteverything.json needs live widget values
                try:
                    backend   = self._vm_backend_var.get() if hasattr(self, "_vm_backend_var") else globals().get("current_vm_backend", "vmware")
                    vm        = self._vm_var.get().strip()  if hasattr(self, "_vm_var")      else globals().get("VMX_PATH", "")
                    watchdog  = bool(self._auto_start_var.get()) if hasattr(self, "_auto_start_var") else False
                    test_mode = bool(self._test_mode_var.get())  if hasattr(self, "_test_mode_var")  else False
                    save_autostart_everything_config(
                        backend=backend, vm=vm,
                        watchdog=watchdog, test_mode=test_mode,
                        os_voting_vm=globals().get("current_os_vm", ""),
                        os_voting_backend=globals().get("current_vm_backend", ""),
                    )
                except Exception:
                    pass
                for fn in [
    ...
    def _build_permissions_tab(self, parent):
        ...
        ttk.Checkbutton(gemini_card,
                        text="Mods-only mode — only moderators can queue !sr and !vr; "
                             "mod controls (!skipsr / !skipvr / !clearsr / !clearvr) still work",
...
if __name__ == '__main__':
    # Force log flush after every message to catch crashes
    ...
    try:
        ...
        try:
            logger.info("Creating GUI root window...")
            logger.handlers[0].flush()
            ...
            logger.critical(f"CRITICAL: GUI initialization failed: {traceback.format_exc()}")
            logger.handlers[0].flush()
            raise
        ...
        if _AUTOSTART_EVERYTHING:
            def _auto_start_everything():
                ...
                # ── Re-open the Video Panel if it was running before the relaunch ──
                if video_config.get("enabled", False):
                    try:
                        time.sleep(2.0)
                        app.root.after(0, app.ensure_video_window)
                        print("[AutoStart] Video Panel re-opened after relaunch.")
                        # ── Bind the OBS video source to the re-opened window ──
                        if OBS_CONFIG.get("enabled", False) and _obs_connected:
                            time.sleep(3.0)
                            if obs_update_video_window_capture("video"):
                                print("[AutoStart] OBS video source bound to re-opened Video Panel.")
                    except Exception:
        ...
        logger.critical(f"FATAL ERROR in main bot: {traceback.format_exc()}")
        _native_dialog_error(
