...
def vm_start(target, backend=None, gui=True):
    ...
    return subprocess.run([VMRUN_PATH, "-T", VMRUN_TARGET_TYPE, "start", target, "gui" if gui else "nogui"], capture_output=True, text=True)

def vm_stop(target, backend=None, hard=True):
    backend = backend or current_vm_backend
    if backend == "vbox":
        return vbox_stop(target, hard, VBOXMANAGE_PATH)
    return subprocess.run([VMRUN_PATH, "-T", VMRUN_TARGET_TYPE, "stop", target, "hard" if hard else "soft"], capture_output=True, text=True)

def vm_reset(target, backend=None):
    backend = backend or current_vm_backend
    if backend == "vbox":
        return vbox_reset(target, VBOXMANAGE_PATH)
    return subprocess.run([VMRUN_PATH, "-T", VMRUN_TARGET_TYPE, "reset", target, "hard"], capture_output=True, text=True)
...
def vm_revert_to_snapshot(target, snapshot_name, backend=None):
    ...
    return subprocess.run([VMRUN_PATH, "-T", VMRUN_TARGET_TYPE, "revertToSnapshot", target, snapshot_name], capture_output=True, text=True)

def vm_take_snapshot(target, snapshot_name, backend=None):
    backend = backend or current_vm_backend
    if backend == "vbox":
        return vbox_take_snapshot(target, snapshot_name, VBOXMANAGE_PATH)
    return subprocess.run([VMRUN_PATH, "-T", VMRUN_TARGET_TYPE, "snapshot", target, snapshot_name], capture_output=True, text=True)
...
def vm_pause():
    if current_vm_backend == "vbox":
        return retry_vbox(lambda: _checked(subprocess.run([VBOXMANAGE_PATH, "controlvm", VMX_PATH, "pause"], capture_output=True, text=True)),
                           source="Cmd/pausevm")
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', VMRUN_TARGET_TYPE, 'pause', VMX_PATH], check=True),
                       source="Cmd/pausevm")
...
def vm_unpause():
    if current_vm_backend == "vbox":
        return retry_vbox(lambda: _checked(subprocess.run([VBOXMANAGE_PATH, "controlvm", VMX_PATH, "resume"], capture_output=True, text=True)),
                           source="Cmd/resumevm")
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', VMRUN_TARGET_TYPE, 'unpause', VMX_PATH], check=True),
                       source="Cmd/resumevm")
...
def vm_save_state():
    ...
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', VMRUN_TARGET_TYPE, 'suspend', VMX_PATH], check=True),
                       source="Cmd/vmsavestate")
...
class UltraBotGUI:
    ...
    def _build_main_tab(self, parent):
        ...
        reg_row.grid(row=4, column=0, columnspan=3, sticky="ew", pady=(22, 0))
