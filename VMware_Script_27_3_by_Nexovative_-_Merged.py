...
GITHUB_SCRIPT_URL  = "https://raw.githubusercontent.com/mrtristin449/TheUltimateYouTubeChatUsesVMsPythonScriptLiveBot247ScriptMrTristin/main/VMware_Script_27_3_by_Nexovative_-_Merged.py"

# Public key used to verify the signature of downloaded updates.
# This is SAFE to keep here — it can only verify signatures, not create them. eeeeeeeeeeeeeeeeeeeeeeeee
...
AUTOUPDATE_SCRIPT_URL  = "https://raw.githubusercontent.com/mrtristin449/TheUltimateYouTubeChatUsesVMsPythonScriptLiveBot247ScriptMrTristin/main/VMware_Script_27_3_by_Nexovative_-_Merged.py"
...
def _load_vm_registry():
    try:
        ...
        print(f"[VM Registry] Load error: {e}")
    return {}
...
def _save_vm_registry(registry):
    try:
        ...
        print(f"[VM Registry] Save error: {e}")
...
def switch_os(target_entry, announce=True):
    ...
    """
    global current_os_vm, VMX_PATH, os_switch_in_progress, os_votes, os_vote_start_time
    ...
    try:
        ...
        if current_os_vm and current_os_vm != target_vm:
            ok, err = retry_vbox(
                lambda: subprocess.run(
                    [VMRUN_PATH, '-T', 'ws', 'stop', current_os_vm, 'hard'], check=True
                ),
        ...
        ok, err = retry_vbox(
            lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'start', target_vm, 'gui'], check=True),
        ...
        if ok:
            ...
            if previous_vm and previous_vm != target_vm:
                ok2, err2 = retry_vbox(
                    lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'start', previous_vm, 'gui'], check=True),
                ...
                if ok2:
                    ...
                    notify("Critical: No VM Running",
                           "Both the target and previous OS failed to start. Check VMware Workstation.",
                           timeout=10)
...
def start_vm():
    try:
        ...
        subprocess.run([VMRUN_PATH, '-T', 'ws', 'start', VMX_PATH, 'gui'], check=True)
        update_status("Running")
        print("[VM] Started!")
    except Exception as e:
        ...
        try:
            ...
            if not is_running:
                if revert_in_progress or os_switch_in_progress:
                    ...
                    ok, err = retry_vbox(
                        lambda: subprocess.run(
                            [VMRUN_PATH, '-T', 'ws', 'start', VMX_PATH, 'gui'], check=True
                        ),
...
def vm_shutdown_soft():
    ok, err = retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'stop', VMX_PATH, 'soft'], check=True),
                          source="Cmd/shutdown")
    if ok:
...
def vm_shutdown_hard_kill():
    ok, err = retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'stop', VMX_PATH, 'hard'], check=True),
                          source="Cmd/killvm")
    if ok:
        obs_trigger("vm_shutdown")
    return ok, err
...
def vm_pause():
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'pause', VMX_PATH], check=True),
                       source="Cmd/pausevm")

def vm_unpause():
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'unpause', VMX_PATH], check=True),
                       source="Cmd/resumevm")

def vm_save_state():
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'suspend', VMX_PATH], check=True),
                       source="Cmd/vmsavestate")

def vm_make_snapshot(name):
    name = (name or "").strip() or time.strftime("snap_%Y%m%d_%H%M%S")
    return retry_vbox(lambda: subprocess.run([VMRUN_PATH, '-T', 'ws', 'snapshot', VMX_PATH, name], check=True),
...
def vm_set_internet_live(enabled, os_type=None, iface=None):
    """Toggles internet for every VM using NAT by starting/stopping the host's
    'VMware NAT Service' via subprocess -- no VNC, no VM restart."""
    try:
...
def vm_is_running():
    try:
        out = subprocess.run([VMRUN_PATH, '-T', 'ws', 'list'], capture_output=True, text=True, timeout=10)
        return VMX_PATH in (out.stdout or "")
    except Exception:
        return False
...
class UltraBotGUI:
    ...
    def _build_main_tab(self, parent):
        ...
        # VM selector
        tk.Label(card, text="VMware .vmx Path", bg=self.BG2,
                 fg=self.TEXTDIM, font=("Segoe UI",9,"bold")).grid(
        ...
        reg_row.grid(row=2, column=0, columnspan=3, sticky="ew", pady=(22, 0))
    ...
    def _vm_restart(self):
        ...
        def run():
            try:
                speak_text("Restarting Virtual Machine...")
                update_status("Restarting...")
                subprocess.run([VMRUN_PATH, '-T', 'ws', 'reset', VMX_PATH, 'hard'], check=True)
                update_status("Running")
                play_success_sound()
                obs_trigger("restart")
                obs_trigger("restart_done")
    ...
    def _vm_revert(self):
        ...
        def run():
            ...
            try:
                speak_text("Reverting Virtual Machine...")
                update_status("Reverting...")
                subprocess.run([VMRUN_PATH, '-T', 'ws', 'stop', VMX_PATH, 'hard'], check=True)
                time.sleep(3)
    ...
    def _refresh_vm_list(self):
        ...
        if vms:
            ...
            self._log(f"VMware: {len(vms)} VM(s) registered.")
        else:
    ...
    def _on_test_mode_toggle(self):
        global TEST_MODE_ENABLED, VMX_PATH, current_os_vm
        ...
        if enabled:
            ...
            if not vm and not (OS_VOTING_ENABLED and OS_LIST):
                messagebox.showerror("Missing VM",
                    "Please select a VMware VM before enabling Test Mode.")
            ...
            self._log(f"[TestMode] Started. VM: {VMX_PATH}. Type commands in the console.")
    ...
    def _start_bot(self):
        global VIDEO_ID, VMX_PATH, current_os_vm
        ...
        if OS_VOTING_ENABLED:
            ...
            if not vm:
                messagebox.showerror("Missing Input", "Please select a VMware VM.")
                return
            VMX_PATH = vm
            current_os_vm = vm
    ...
    def _vnc_test_connect(self):
        ...
        def _try():
            try:
                ...
                self._log(f"[VNC] Connection failed: {e}")
