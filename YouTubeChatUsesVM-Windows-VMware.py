...
def vm_start(target, backend=None, gui=True):
    ...
    return subprocess.run([VMRUN_PATH, "-T", "ws", "start", target, "gui" if gui else "nogui"], capture_output=True, text=True)

def vm_stop(target, backend=None, hard=True):
    backend = backend or current_vm_backend
    if backend == "vbox":
        return vbox_stop(target, hard, VBOXMANAGE_PATH)
    return subprocess.run([VMRUN_PATH, "-T", "ws", "stop", target, "hard" if hard else "soft"], capture_output=True, text=True)

def vm_reset(target, backend=None):
    backend = backend or current_vm_backend
    if backend == "vbox":
        return vbox_reset(target, VBOXMANAGE_PATH)
    return subprocess.run([VMRUN_PATH, "-T", "ws", "reset", target, "hard"], capture_output=True, text=True)
...
def vm_revert_to_snapshot(target, snapshot_name, backend=None):
    ...
    return subprocess.run([VMRUN_PATH, "-T", "ws", "revertToSnapshot", target, snapshot_name], capture_output=True, text=True)

def vm_take_snapshot(target, snapshot_name, backend=None):
    backend = backend or current_vm_backend
    if backend == "vbox":
        return vbox_take_snapshot(target, snapshot_name, VBOXMANAGE_PATH)
    return subprocess.run([VMRUN_PATH, "-T", "ws", "snapshot", target, snapshot_name], capture_output=True, text=True)
